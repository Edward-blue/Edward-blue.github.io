function Base() {
  ...
  this.loadingBeforeInit = function () {
    ...
    // 添加扩展字体图标库
    if (window.config.FontIconExtend !== '')
      tools.dynamicLoadingCss(window.config.FontIconExtend)
  ...
  this.loadingAfterInit = function () {
    // 设置右下角菜单事件
    timers.setWrapRightEventTimer = window.setInterval(
      script.setWrapRightEvent,
      1000
    )
    ...
    // 归档页面动画
    timers.setArchiveMotionTimer = window.setInterval(
      script.setArchiveMotion,
      1000
    )
  ...
  this.setPageAnimationControl = function () {
    if (
      window.config.HomeHeaderAnimationRendered ||
      window.config.ArticleHeaderAnimationRendered ||
      window.config.BackAnimationRendered
    ) {
      let html =
        '<div id="animationSwitch" data="off" style="z-index: 999; position: absolute; top: 15px; right: 20px; font-size: 14px; color: #F9F9F9; cursor: pointer;">'
      html +=
        '<span class="iconfont icon-lightning" style="display: inline-block;"></span>'
    ...
    if (wrapRight.length > 0) {
      ...
      $('.wrap-right .favorites').on('click', function () {
        if ($(this).attr('switch') == 'false') {
          ...
          $(this)
            .find('.iconfont')
            .removeClass('icon-favorites')
            .addClass('icon-favorites-fill')
          $(this).find('.icontext').text('已关注')
        } else {
          $(this).attr('switch', 'false')
          $(this)
            .find('.iconfont')
            .removeClass('icon-favorites-fill')
            .addClass('icon-favorites')
      ...
      window
        ...
        .addListener(modeListeners.light)
      window
        .matchMedia('(prefers-color-scheme: dark)')
        .addListener(modeListeners.dark)
  ...
  this.homeInit = function () {
    // 设置主页图片
    let homeHeaderImage = window.config.HomeHeaderImage,
      backImage

    homeHeaderImage.length > 0
      ? homeHeaderImage.length > 1
        ? (backImage =
            homeHeaderImage[tools.randomNum(0, homeHeaderImage.length - 1)])
        : (backImage = homeHeaderImage[0])
      : (backImage = '')
    $('#header').css({
  ...
  this.setTocPositionFixed = function () {
    if (window.config.Tocbot.switch && $('.toc').length > 0) {
      ...
      if (docScroll > header) {
        $(window.config.Tocbot.positionFixedSelector).addClass(
          window.config.Tocbot.positionFixedClass
        )
      } else {
        $(window.config.Tocbot.positionFixedSelector).removeClass(
          window.config.Tocbot.positionFixedClass
        )
      }
    }
  ...
  this.setOtherHeaderImage = function () {
    ...
    articleHeaderImage.length > 0
      ? articleHeaderImage.length > 1
        ? (backImage =
            articleHeaderImage[
              tools.randomNum(0, articleHeaderImage.length - 1)
            ])
        : (backImage = articleHeaderImage[0])
      : (backImage = '')
      ...
      if (codeBlock.length > 0) {
        codeBlock.each(function (i, block) {
          ...
          $(block)
            ...
            .css(
              'width',
              'calc(100% - ' + (lineNumWidth ? lineNumWidth : 0) + 'px)'
            )
        })
      }
    }
      ...
      require(['Clipboard'], function (Clipboard) {
        pre.each(function (i) {
          ...
          object.wrap(
            '<code-box id="' +
              id +
              '" style="position: relative; display: block;"></code-box>'
          )
          ...
          let html =
            '<button code-id="' +
            id +
            '" type="button" class="clipboard" data-clipboard-action="copy" data-clipboard-target="pre[code-id=\'' +
            id +
            '\']" aria-label="复制代码"><i class="iconfont icon-copy"></i></button>'
        ...
        $('code-box button').click(function () {
          ...
          setTimeout(
            '$("code-box button[code-id=' +
              $(this).attr('code-id') +
              '] i").removeClass("icon-check").addClass("icon-copy")',
            1500
          )
        })
    ...
    if (window.config.Valine.switch) {
      require(['LeanCloud', 'Valine'], function (LeanCloud, Valine) {
        ...
        let valine = new Valine({
          el: '#vcomments',
          appId: 'fDSPJPy5L1cRj5bgGTNOu8ZR-gzGzoHsz',
          appKey: 'CGD4Cer86gcXXDlCAVrwK20g',
        })
